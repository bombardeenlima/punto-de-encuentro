<script lang="ts">
  ...
  const viewBox = $derived.by(() => `${viewBounds.minX} ${viewBounds.minY} ${viewBounds.span} ${viewBounds.span}`);
  ...
  {#if interactive}
    ...
    >
      ...
      >
        ...
        <rect x={viewBounds.minX} y={viewBounds.minY} width={viewBounds.span} height={viewBounds.span} fill="url(#plane-background)" />

        <g class="stroke-border/60">
          {#each gridLines.vertical as value (value)}
            {#if Math.abs(value) > 1e-6}
              <line x1={value} y1={-viewBounds.maxY} x2={value} y2={-viewBounds.minY} stroke-width={gridStrokeWidth} />
            {/if}
          {/each}
          {#each gridLines.horizontal as value (value)}
            {#if Math.abs(value) > 1e-6}
              <line x1={viewBounds.minX} y1={-value} x2={viewBounds.maxX} y2={-value} stroke-width={gridStrokeWidth} />
            {/if}
          {/each}
        </g>
        ...
        <g class="stroke-border/90">
          {#if axesInView.yAxis}
            <line x1={0} y1={-viewBounds.maxY} x2={0} y2={-viewBounds.minY} stroke-width={axisStrokeWidth} />
          {/if}
          {#if axesInView.xAxis}
            <line x1={viewBounds.minX} y1={0} x2={viewBounds.maxX} y2={0} stroke-width={axisStrokeWidth} />
          {/if}
        </g>
        ...
        {#if axesInView.xAxis}
          <g class="fill-muted-foreground" font-size={tickFontSize}>
            {#each majorTicks.y as tick (tick)}
              {#if Math.abs(tick) > 1e-6}
                <text x={tickLabelOffset} y={-tick} text-anchor="start" alignment-baseline="middle">{formatTick(tick)}</text>
              {/if}
            {/each}
          </g>
        {/if}
        ...
        <g class="fill-muted-foreground/80" font-size={axisFontSize}>
          {#if axesInView.yAxis}
            <text x={viewBounds.maxX - axisLabelOffset} y={-axisLabelOffset} text-anchor="end">x</text>
          {/if}
          {#if axesInView.xAxis}
            <text x={axisLabelOffset} y={-(viewBounds.maxY - axisLabelOffset)} text-anchor="start">y</text>
          {/if}
        </g>
